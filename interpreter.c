...
// Reccomended order of implementation of instructions: 
// 1. addX, haltX, nlX, doutX
// 2. andX, notX
// 3. ldX, stX
// 4. ldrX, strX, leaX
// 5. brX, brnX, brpX, brzX
// 6. blX, blrX, jmp
...
int main(int argc, char *argv[])
{
   ...
   while (1)
   {
      ...
      switch (opcode)
      {
         ...
         case 1:                               // add
            if (bit5)                           //bit five determines which type of add we are looking at. if (bit5) is checking if bit5 is nonzero.
            {
               regsave1 = r[sr1];
               r[dr] = regsave1 + imm5;
               // set c, v flags
         ...
         case 2:                          // ld       did in class
            r[dr] = mem[pc + pcoffset9];
            break;
                    
         case 3:                          //st        opposite of ld, only difference is sr vs dr
            mem[pc + pcoffset9] = r[sr];    
            break; 

         case 4:                          //bl, blr
            if(code == 0){                 //if blr or jsrr    
               r[7] = pc;
               pc = r[baser] + offset6;
               break;
            }else{                        //if bl, call, or jsr
               r[7] = pc;                 //r7 is link register
               pc = pc + pcoffset11;
               break; 
            }
         ...
         case 15:                         // trap
            ...
            if (trapvec == 0x02)             // dout
               printf("%d", r[dr]);          //if this is issue could be sr? 

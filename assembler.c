...
{  
   ...
   printf("error %s at line num %d : %s \n", p, linenum, linesave );
...
unsigned short getadd(char *p)
{
   ...
   error("Symbol table does not contain p.");      //catch 
   return 0;
...
int main(int argc,char *argv[])
{
   ...
   while (fgets(buf, sizeof(buf), infile))
   {
      ...
      if (!isspace(buf[0]))         // line starts with label
      {
         ...
         for(int i = 0; i < stsize; i++){
            if(!mystrcmpi(label, symbol[i])){       //if this label already exists in symbol table
               error("This label is a duplicate.");      //error the label xD 
           
         }
      }
         symbol[stsize] = label;             //if this breaks try moving these lines out of for loop? 
         symadd[stsize++] = loc_ctr;         //                          these
   ...
   while (fgets(buf, sizeof(buf), infile))
   {
      ...
      //most of this pulled from ld
         (!mystrcmpi(mnemonic, "st")){
            sr = getreg(o1) << 9;                        //find sr in line
            pcoffset9 = (getadd(o2) - loc_ctr - 1);      //calculate pcoffset9
            if(pcoffset9 > 255 || pcoffset9 < -256){     //pcoffset9 valid? if not: error
               error("pcoffset9 out of range ");
            }
            macword = 0x3000 | sr | (pcoffset9 & 0x1ff); //assemble inst or whatever
            fwrite(&macword, 2, 1, outfile);             //write out inst
            loc_ctr++;                                   
         }
       else if 
         (!mystrcmpi(mnemonic, "bl")){
            pcoffset11 = (getadd(o1) - loc_ctr -1);      //calc pcoffset11
            if(pcoffset11 > 1023 || pcoffset11 < -1024){ //pcoffset11 valid? if not throw error
         ...
         (!mystrcmpi(mnemonic, "blr")){      //frankensteined from jmp
            baser = getreg(o1) << 6;
            if (o2 && mystrcmpi(o2, ";"))                         // offset6 specified, and not a comment 
         {
         ...
         macword = 0x4000 | baser | (num) ;
         fwrite(&macword, 2, 1, outfile);
         loc_ctr++;

         }
       else if 
         //frankensteined from add
         (!mystrcmpi(mnemonic, "and")){
            if (!o3){
               error("Missing o3");
            }
         dr = getreg(o1) << 9;   // get and position dest reg number
       ...
       //frankensteined from ld and and/add
         (!mystrcmpi(mnemonic, "ldr")){
            dr = getreg(o1) << 9;// get and position destination reg number
            baser = getreg(o2) << 6;
            if(o3){
               if (sscanf(o3,"%d", &num) != 1){    // check offset6 
                  error("Bad offset6");
               }   
            if (num > 31 || num < -32){
               error("offset6 out of range");
               ...
               num = 0;          //default offset to zero if none given
            }
       ...
       //frankensteined from ldr
         (!mystrcmpi(mnemonic, "str")){
            sr = getreg(o1) << 9;  
            baser = getreg(o2) << 6;
         ...
         (!mystrcmpi(mnemonic, "not")){
            ...
            fwrite(&macword, 2, 1, outfile);          //try saying "fwrite" out loud. do you say it f-write or fright
            loc_ctr++;                                //just figured i'd ask.
      ...
      if (!mystrcmpi(mnemonic, "ret" ))     // also ret instruction
      {
         ...
         baser = 7 << 6;            //baser is always 7 as stated above 
         if(o1){
            if(sscanf(o1, "%d", &num) != 1){       //checking
               error("Bad offset");
            }
            if(num > 31 || num < -32){
               error("offset6 out of range");
         ...
         if (pcoffset9 > 255 || pcoffset9 < -256)
            error("pcoffset9 out of range");
         macword = 0xe000 | dr | (pcoffset9 & 0x01ff);
         fwrite(&macword, 2, 1, outfile);
         loc_ctr++;
         ...
         if(o1 == NULL){
            ...
            if(!mystrcmpi(o1, ";")){      //if no o1 declared but we have ; we must set to default
               sr = 0;
            } else {
               sr = getreg(o1) << 9;
            }
         ...
         sscanf(o1, "%x", &num);                      // hex print i forgot about
         macword = num;
         fwrite(&macword, 2, 1, outfile);
         loc_ctr++;
      }
